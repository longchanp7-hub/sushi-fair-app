...
function renderTodayHighlights(allChains) {
  ...
  if (startsToday.length) {
    blocks.push(`<div class="highlight-group hot"><div class="highlight-title"><span>🔥</span><strong>本日スタート</strong><em>${startsToday.length}件</em></div>${startsToday.map(fair => highlightCard(fair, '今日から')).join('')}</div>`);
  }
  if (endingSoon.length) {
    blocks.push(`<div class="highlight-group soon"><div class="highlight-title"><span>⏰</span><strong>もうすぐ終了</strong><em>${endingSoon.length}件</em></div>${endingSoon.map(({ fair, days }) => highlightCard(fair, days === 0 ? '本日まで' : days === 1 ? '明日まで' : `あと${days}日`)).join('')}</div>`);
  }
  if (!blocks.length) {
    const active = allChains.slice(0, 2);
    blocks.push(`<div class="highlight-group calm"><div class="highlight-title"><span>🍣</span><strong>今日の注目</strong></div>${active.map(fair => highlightCard(fair, remainingLabel(fair.endDate))).join('')}</div>`);
  }
...
function highlightCard(fair, label) {
  ...
  return `<a class="highlight-card" href="${escapeHtml(fair.sourceUrl || '#')}" target="_blank" rel="noopener noreferrer"><div><span class="highlight-chain">${escapeHtml(meta.name)}</span><strong>${escapeHtml(fair.fairName || first?.name || '期間限定メニュー')}</strong>${first ? `<small>${escapeHtml(first.name)}${first.price ? ` ・ ${Number(first.price).toLocaleString('ja-JP')}円` : ''}</small>` : ''}</div><span class="highlight-deadline">${escapeHtml(label)}</span></a>`;
}
...
function render() {
  ...
  summaryEl.innerHTML = allChains.map(fair => {
    ...
    return `<a class="chain-summary" href="${escapeHtml(fair.sourceUrl || '#')}" target="_blank" rel="noopener noreferrer" style="--chain-color:${meta.color}"><div class="chain-summary-media">${image}<span class="chain-summary-fallback">🍣</span></div><div class="chain-summary-body"><div><strong>${escapeHtml(meta.name)}</strong><span>${count}品</span></div><small>${escapeHtml(fair.fairName || '開催中フェア')}</small></div><span class="chain-summary-arrow">↗</span></a>`;
  }).join('');

  cardsEl.innerHTML = fairs.map(fair => {
    const meta = chainMeta[fair.chain];
    const items = (fair.items || []).slice(0, 8);
    const itemHtml = items.length ? `<ul class="items">${items.map(item => `<li class="item"><span class="item-name">${escapeHtml(item.name)}</span><span class="price">${item.price ? `${Number(item.price).toLocaleString('ja-JP')}円` : '—'}</span></li>`).join('')}</ul>${(fair.items?.length || 0) > 8 ? `<div class="more">ほか ${fair.items.length - 8} 品</div>` : ''}` : `<div class="empty">商品一覧の自動取得に失敗しました。公式ページから確認できます。</div>`;
    return `<article class="card" style="--chain-color:${meta.color}"><div class="card-top"><div><div class="chain">${meta.name}</div><div class="store">${escapeHtml(fair.storeName || '')}</div></div><span class="badge">${remainingLabel(fair.endDate)}</span></div><h2>${escapeHtml(fair.fairName || '期間限定メニュー')}</h2><p class="period">${periodText(fair)}</p>${fair.status !== 'ok' ? `<div class="error-banner">${escapeHtml(fair.message || '最新情報の一部を取得できませんでした。')}</div>` : ''}${itemHtml}<div class="actions"><a class="primary" href="${fair.sourceUrl}" target="_blank" rel="noopener">公式で確認</a>${fair.storeUrl ? `<a class="secondary" href="${fair.storeUrl}" target="_blank" rel="noopener">店舗</a>` : ''}</div></article>`;
  }).join('');
}
...
async function loadData() {
  ...
  try {
    const res = await fetch(`./data/fairs.json?v=20260715-2115-${Date.now()}`, { cache: 'no-store' });
...
clearLegacyCaches().finally(loadData);
