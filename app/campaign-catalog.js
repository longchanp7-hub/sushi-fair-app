// Pure shared model: collection-time status is never trusted across a JST date boundary.
...
export function renderCampaignCatalog(chain, today = catalogToday()) {
  ...
  const card = c => {
    ...
    const dates=c.startDate ? `${c.startDate}${c.endDate&&c.endDate!==c.startDate?` 〜 ${c.endDate}`:c.endDate?'（当日限定）':' 〜 終了日未確認'}` : '販売・実施期間は公式で確認';
  ...
  return `<section class="campaign-catalog" aria-label="開催中・開始予定の公式告知"><h3>公式の関連キャンペーン</h3>${warning}${active.length?`<h4>開催期間内・期間要確認</h4>${active.map(card).join('')}`:''}${next.length?`<h4>近日開始</h4>${next.map(card).join('')}`:''}${!rows.length?'<p>掲載できる告知は未確認です。</p>':''}<small>一覧照合は告知の収録確認です。商品・価格の完全取得や店舗在庫を保証するものではありません。</small></section>`;
}
