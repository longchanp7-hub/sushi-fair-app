...
const FALLBACK={'北海道':['札幌市中央区'],'東京都':['新宿区','渋谷区','千代田区'],'愛知県':['豊橋市','豊川市','蒲郡市','名古屋市中区'],'大阪府':['大阪市北区','大阪市中央区'],'福岡県':['福岡市博多区'],'沖縄県':['那覇市']};
...
function matchLabel(match){return match==='same_municipality'?'同一市区町村':match==='same_parent_city'?'同一市内':'同一都道府県の代表';}
...
export async function initRegionSelector({onApply}={}){
  ...
  if(!data[applied.prefecture]?.includes(applied.city)){applied=makeLocation(prefs[0],data[prefs[0]][0]);save(applied);}
