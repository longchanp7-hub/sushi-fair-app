...
function findExact(catalog,chain,prefecture,municipality){
  return Object.values(catalog?.[chain]||{}).find(x=>x.prefecture===prefecture&&x.municipality===municipality)||null;
}

function findCity(catalog,chain,prefecture,cityPrefix){
  return Object.values(catalog?.[chain]||{}).find(x=>x.prefecture===prefecture&&String(x.municipality||'').startsWith(cityPrefix))||null;
}
...
async function verifyOnce(a,localIndex,localNational,localFairs,localStores){
  ...
  assert.equal(normalize(nationalText),normalize(localNational));
  validateNational(nationalText);
  const fairsText=await fetchText(new URL('data/fairs.json',base).href,a);
  assert.equal(normalize(fairsText),normalize(localFairs));
  const fairs=JSON.parse(fairsText);validateFairs(fairs);
  const storesText=await fetchText(new URL('data/store-contexts.json',base).href,a);
  assert.equal(normalize(storesText),normalize(localStores));
  const stores=JSON.parse(storesText);validateStores(stores);
  return{
    status:'ok',verifiedAt:new Date().toISOString(),testedCommit:process.env.GITHUB_SHA||null,pageUrl:base.href,
    publicIndexMatchesSource:true,publicNationalMatchesSource:true,publicFairsMatchSource:true,publicStoreContextsMatchSource:true,
    fairsUpdatedAt:fairs.updatedAt,storeContextsUpdatedAt:stores.updatedAt,
    storeContextCounts:Object.fromEntries(Object.entries(stores.catalog||{}).map(([k,v])=>[k,Object.keys(v||{}).length])),
    chains:Object.fromEntries(fairs.chains.map(x=>[x.chain,{fairName:x.fairName,itemCount:x.items.length,status:x.status,representativeImage:Boolean(x.representativeImageUrl||x.imageUrl)}]))
  };
}
...
for(let a=1;a<=MAX_ATTEMPTS;a++){
  try{
    const r=await verifyOnce(a,localIndex,localNational,localFairs,localStores);
    await report(r);
    ...
    last=e;
    console.warn(`Smoke attempt ${a}/${MAX_ATTEMPTS} failed: ${e.message}`);
    if(a<MAX_ATTEMPTS)await sleep(RETRY_DELAY_MS);
  }
}
