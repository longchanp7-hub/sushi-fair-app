...
function actualWaitResult(store, business, wait, detail) {
  if (wait.waitMinutes != null) {
    return withBusiness({
      ...
      label:wait.waitMinutes === 0 ? '待ちなし' : `約${wait.waitMinutes}分待ち`, detail,
  ...
  return withBusiness({
    ...
    label:wait.waitGroups === 0 ? '待ちなし' : `${wait.waitGroups}組待ち`, detail,
    reservationUrl:store.reservationUrl, status:'ok',
  }, business);
}
...
async function getSushiroOrHama(store) {
  ...
  for (const page of pages) {
    ...
    if (wait) return actualWaitResult(store, business, wait, '公式受付・予約ページの公開表示から取得');
  }

  const detail = store.chain === 'sushiro'
    ? '公式画面には待ち状況欄がありますが、公開HTMLから店舗全体の待ち時間を安定取得できる値は確認できていません。'
