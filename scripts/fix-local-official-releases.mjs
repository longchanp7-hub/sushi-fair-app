...
const RELEASE={
  ...
  endDate:'2026-11-03',
  officialReleaseUrl:'https://www.nigirinotokubei.com/info/11195/',
  companyReleaseUrl:'https://origin.digitalpr.jp/r/141163',
...
export function applyOfficialRelease(data,now=today()){
  ...
  fair.status='ok';
  fair.message=null;
  fair.dataScope='local_official_brand_release';
  // The brand page was updated after the company release and its campaign image
  // independently confirms さんま天 410円（税込451円）. Prefer the newer,
  // brand-specific official source when the two official sources disagree.
  fair.officialReleaseUrl=RELEASE.officialReleaseUrl;
  fair.productSourceUrl=RELEASE.officialReleaseUrl;
  fair.companyReleaseUrl=RELEASE.companyReleaseUrl;
  fair.imageUrl=RELEASE.imageUrl;
  fair.representativeImageUrl=RELEASE.imageUrl;
  fair.representativeImageSource='official_brand_campaign_image';
  fair.representativeImageProduct='さんま食べ比べ三貫';
  fair.representativeImagePage=RELEASE.officialReleaseUrl;
  fair.priceNote='にぎりの徳兵衛公式ページ・公式フェア画像の税込価格です。一部店舗では価格が異なります。';
  return true;
}
...
if(process.argv.includes('--self-test')){
  ...
  const f=d.chains[0];
  if(f.items.length!==4||f.items.find(x=>x.name==='さんま天')?.price!==451)throw new Error('Tokubei official price regression');
  if(f.items.find(x=>x.name==='さんま')?.price!==407)throw new Error('Tokubei sanma price regression');
  if(f.productSourceUrl!==RELEASE.officialReleaseUrl||f.representativeImageUrl!==RELEASE.imageUrl)throw new Error('Tokubei official source/image regression');
  ...
  if(applyOfficialRelease(data)){
    ...
    console.log('Applied Tokubei verified brand-official campaign data.');
